...
function lifecycle(kind: string): LifecycleStep {
  ...
  if (kind === "trade.submitted") return "execute";
...
export function toEvent(event: RunEvent, receipts: Receipt[] = []): AgentEvent {
  ...
  const purchase: Purchase | undefined = event.kind === "payment.settled" && receipt?.kind === "data_purchase" ? {
    id: receipt.id,
    ...
    costLabel: `${number(receipt.amountHbar).toFixed(8).replace(/\.?0+$/, "")} HBAR`,
    asset: "HBAR",
    txHash: receipt.transactionId ?? "",
    provenance: receipt.provenance === "stale" ? "fallback" : receipt.provenance ?? "live",
    ms: 0,
  } : undefined;
  const settlement: Settlement | undefined = event.kind === "trade.verified" && receipt?.kind === "trade" && receipt.transactionId ? {
    txHash: receipt.transactionId,
    status: receipt.status === "confirmed" ? "confirmed" : "submitted",
    feeUsd: 0,
    submittedAt: at(receipt.occurredAt),
    ...(receipt.status === "confirmed" ? { confirmedAt: at(receipt.occurredAt) } : {}),
  } : undefined;
  return {
    id: event.id,
    step: lifecycle(event.kind),
    at: at(event.occurredAt), title: event.title ?? event.kind.replace(/[._]/g, " "), detail: event.detail,
    provenance: event.provenance === "stale" ? "fallback" : event.provenance,
    tone: event.kind.includes("failed") || event.kind.includes("blocked") ? "orange" : event.kind.includes("settled") || event.kind.includes("verified") ? "green" : event.kind.includes("payment") ? "signal" : "ink",
    purchase,
    settlement,
    price: number(payload.price ?? payload.usdPrice),
  } as AgentEvent & { price: number };
}
